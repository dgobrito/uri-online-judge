...
public interface EmpregadoRepository extends JpaRepository<Empregado, Long> {
	...
	@Query(value = "SELECT empregados.cpf, empregados.enome, departamentos.dnome FROM empregados "
				 ...
				 + ") "
				 + "ORDER BY empregados.cpf", nativeQuery = true)
	List<EmpregadoDeptProjection> search1();
	
	@Query(value = "SELECT new com.devsuperior.uri2990.dto.EmpregadoDeptDTO(obj.cpf, obj.enome, obj.departamento.dnome) FROM Empregado obj "
			 + "WHERE "
			 + "obj.cpf NOT IN ("
	...
	@Query(value = "SELECT new com.devsuperior.uri2990.dto.EmpregadoDeptDTO(obj.cpf, obj.enome, obj.departamento.dnome) FROM Empregado obj "
				 + "LEFT JOIN obj.projetosOndeTrabalha prj "
				 + "WHERE "
				 + "prj.pnumero IS NULL "
				 + "ORDER BY obj.cpf")
	List<EmpregadoDeptDTO> search4();	
}